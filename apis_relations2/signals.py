...
def find_relationtype(tt: TempTriple):
    ...
    for model in apps.get_models():
        if getattr(model, 'obj_model', None) == obj_model and getattr(model, 'subj_model', None) == subj_model and getattr(model, 'temptriple_name', None) == name and getattr(model, 'temptriple_name_reverse', None) == reverse:
